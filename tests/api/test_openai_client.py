...
class TestConvertMessagesToOpenai:
    ...
    def test_no_system_prompt(self):
        messages = [PlatformMessage.user("hi")]
    ...
    def test_assistant_with_tool_calls(self):
        msg = PlatformMessage(
            ...
            content=[
                ...
                ToolCallBlock(id="call_1", name="read_file", input={"path": "/tmp/x"}),
            ],
        )
        result = _convert_messages_to_openai([msg], None)
        assert result[0]["role"] == "assistant"
    ...
    def test_full_conversation_round_trip(self):
        ...
        messages = [
            PlatformMessage.user("Read /tmp/test.txt"),
            PlatformMessage(
                role="assistant",
                content=[
...
async def test_openai_client_uses_full_base_url_path_for_requests():
    ...
    request = ApiMessageRequest(
        model="gpt-4o-mini",
        messages=[PlatformMessage.user("Explain the codebase")],
    )
...
def test_openai_client_init_normalizes_base_url(monkeypatch):
    ...
    monkeypatch.setattr("api.openai_client.AsyncOpenAI", _StubAsyncOpenAI)
...
class TestReasoningContentEmission:
    ...
    requires opt-in via ``CODEHARNESS_REQUIRE_EMPTY_REASONING_CONTENT=1``.
    ...
    def _msg_with_tool_use(self, *, reasoning: str | None = None) -> PlatformMessage:
        msg = PlatformMessage(
            role="assistant",
            content=[
                TextBlock(text="ok"),
                ToolCallBlock(id="tool_1", name="read_file", input={"path": "x"}),
            ],
        )
    ...
    def test_omits_reasoning_when_no_captured_text(self, monkeypatch):
        monkeypatch.delenv("CODEHARNESS_REQUIRE_EMPTY_REASONING_CONTENT", raising=False)
    ...
    def test_emits_empty_when_opted_in(self, monkeypatch):
        monkeypatch.setenv("CODEHARNESS_REQUIRE_EMPTY_REASONING_CONTENT", "1")
        out = _convert_assistant_message(self._msg_with_tool_use())
        assert out["reasoning_content"] == ""

    def test_opt_in_truthy_values(self, monkeypatch):
        for v in ("1", "true", "TRUE", "yes", "on"):
            monkeypatch.setenv("CODEHARNESS_REQUIRE_EMPTY_REASONING_CONTENT", v)
            out = _convert_assistant_message(self._msg_with_tool_use())
    ...
    def test_no_tool_calls_never_emits_empty(self, monkeypatch):
        ...
        monkeypatch.setenv("CODEHARNESS_REQUIRE_EMPTY_REASONING_CONTENT", "1")
        msg = PlatformMessage(role="assistant", content=[TextBlock(text="hi")])
