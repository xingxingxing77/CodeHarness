...
from api.client import ApiMessageRequest, ApiMessageCompleteEvent, ApiTextDeltaEvent
from api.codex_client import (
    CodexApiClient,
...
def test_convert_messages_to_codex():
    messages = [
        PlatformMessage.user("Inspect file"),
        PlatformMessage(
            role="assistant",
            content=[
                TextBlock(text="I'll inspect it."),
                ToolCallBlock(id="call_123", name="read_file", input={"path": "README.md"}),
            ],
        ),
        PlatformMessage(
            role="user",
...
async def test_codex_client_streams_text(monkeypatch):
    ...
    monkeypatch.setattr(
        "api.codex_client.httpx.AsyncClient",
...
async def test_codex_client_emits_tool_use(monkeypatch):
    ...
    request = ApiMessageRequest(
        model="gpt-5.4",
        messages=[PlatformMessage.user("glob")],
    ...
    tool_use = complete.message.tool_calls[0]
