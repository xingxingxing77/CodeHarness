...
适配（api内核搬迁细则 §5）：持久化层由文件函数改为 TokenStore 协议；
device flow 测试保持供体语义。
"""
...
class TestMemoryTokenStore:
    def test_round_trip(self):
        ...
        store.save(CopilotAuthInfo(github_token="gho_abc123"))
        info = store.load()
        assert info is not None and info.github_token == "gho_abc123"
        assert info.enterprise_url is None

    def test_clear(self):
        store = MemoryTokenStore()
        store.save(CopilotAuthInfo(github_token="gho_xyz"))
        store.clear()
        assert store.load() is None

    def test_clear_noop_when_empty(self):
        MemoryTokenStore().clear()  # should not raise


class TestFileTokenStore:
    def test_save_and_load_round_trip(self, tmp_path: Path):
        store = FileTokenStore(tmp_path / "cfg" / "copilot_auth.json")
        store.save(CopilotAuthInfo(github_token="gho_abc123"))
        info = store.load()
    ...
    def test_save_and_load_with_enterprise_url(self, tmp_path: Path):
        store = FileTokenStore(tmp_path / "cfg" / "copilot_auth.json")
        store.save(CopilotAuthInfo(github_token="gho_ent_token", enterprise_url="company.ghe.com"))
        info = store.load()
        assert info is not None
...
class TestRequestDeviceCode:
    ...
    def test_request_device_code_happy_path(self, monkeypatch):
        ...
        monkeypatch.setattr("api.copilot_auth.httpx.post", fake_post)
...
class TestPollForAccessToken:
    ...
    def test_returns_token_after_pending(self, monkeypatch):
        ...
        monkeypatch.setattr("api.copilot_auth.httpx.post", fake_post)
        monkeypatch.setattr("api.copilot_auth.time.sleep", lambda _: None)
    ...
    def test_slow_down_increases_interval(self, monkeypatch):
        ...
        monkeypatch.setattr("api.copilot_auth.httpx.post", fake_post)
        monkeypatch.setattr("api.copilot_auth.time.sleep", fake_sleep)
    ...
    def test_timeout_raises_runtime_error(self, monkeypatch):
        ...
        monkeypatch.setattr("api.copilot_auth.httpx.post", fake_post)
        monkeypatch.setattr("api.copilot_auth.time.sleep", lambda _: None)
        monkeypatch.setattr("api.copilot_auth.time.monotonic", fake_monotonic)
