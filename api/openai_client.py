...
_EMPTY_REASONING_ENV = "CODEHARNESS_REQUIRE_EMPTY_REASONING_CONTENT"
...
def _convert_assistant_message(msg: PlatformMessage) -> dict[str, Any]:
    """Convert an assistant PlatformMessage to OpenAI format.
    ...
    - When the streaming parser captured non-empty reasoning on
      ``msg.reasoning``, we always replay it. Models that emit reasoning
    ...
    - When there is no captured reasoning but the message has tool calls,
      ...
      ``CODEHARNESS_REQUIRE_EMPTY_REASONING_CONTENT=1``. The default is
    ...
    tool_uses = [b for b in msg.content if isinstance(b, ToolCallBlock)]
...
def _parse_assistant_response(response: Any) -> PlatformMessage:
    """Parse an OpenAI ChatCompletion response into a PlatformMessage."""
    ...
    return PlatformMessage(role="assistant", content=content)
...
class OpenAICompatibleClient:
    ...
    async def _stream_once(self, request: ApiMessageRequest) -> AsyncIterator[ApiStreamEvent]:
        ...
        # Build the final PlatformMessage
        content: list[ContentBlock] = []
        ...
        final_message = PlatformMessage(role="assistant", content=content)
        ...
        if collected_reasoning:
            final_message.reasoning = collected_reasoning
