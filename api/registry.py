...
def _match_by_model(model: str) -> ProviderSpec | None:
    ...
    std_specs = [s for s in PROVIDERS if not s.is_local]  # OAuth 供应商也参与模型名匹配（codex 经此路由）
...
# ---------------------------------------------------------------------------
# 平台扩展点（api内核搬迁细则 §3-7）：注册自定义供应商，不改内核
# ---------------------------------------------------------------------------


def register_provider(spec: ProviderSpec, *, before: str | None = None) -> None:
    """注册供应商规格。before=某已有 spec.name 时插到它之前（优先级语义），否则追加尾部。"""
    global PROVIDERS
    if before is not None:
        index = next((i for i, s in enumerate(PROVIDERS) if s.name == before), len(PROVIDERS))
        PROVIDERS = (*PROVIDERS[:index], spec, *PROVIDERS[index:])
    else:
        PROVIDERS = (*PROVIDERS, spec)


# Codex 订阅通道（ChatGPT JWT）：供体经 api_format/provider 字符串分支路由，
# 平台改为注册表驱动，补一个显式规格。
register_provider(
    ProviderSpec(
        name="codex",
