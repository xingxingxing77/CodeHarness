...
from api.errors import AuthenticationFailure, OpenHarnessApiError, RateLimitFailure, RequestFailure
from api.usage import UsageSnapshot
from engine.messages import PlatformMessage, ImageBlock, TextBlock, ToolResultBlock, ToolCallBlock
...
def _convert_messages_to_codex(messages: list[PlatformMessage]) -> list[dict[str, Any]]:
    result: list[dict[str, Any]] = []
    for msg in messages:
        if msg.role == "user":
            # Responses API requires function_call_output items to appear before
            # any following user input.  A PlatformMessage can contain both
...
class CodexApiClient:
    ...
    def __init__(
        ...
        *,
        ...
        self._http = http_client  # 注入则跨请求复用连接池；None 则保持供体逐请求语义
    ...
    async def _stream_once(self, request: ApiMessageRequest) -> AsyncIterator[ApiStreamEvent]:
        ...
        client_ctx = (
            contextlib.nullcontext(self._http)
            if self._http is not None
            else httpx.AsyncClient(timeout=60.0, follow_redirects=True)
        )
                    ...
                    if event_type == "response.output_text.delta":
                        ...
                        if item_type == "message":
                            ...
                            if isinstance(call_id, str) and call_id and isinstance(name, str) and name:
                                content.append(ToolCallBlock(id=call_id, name=name, input=parsed_arguments))
        ...
        stop_reason = _stop_reason_from_response(
            ...
            has_tool_calls=bool(final_message.tool_calls),
        )
